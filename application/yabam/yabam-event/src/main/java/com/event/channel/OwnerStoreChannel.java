...
public class OwnerStoreChannel {
	...
	private static final String CHANNEL_NAME = "OwnerStoreChannel";
	private final EmitterGenerator emitterGenerator;

	public SseEmitter subscribe(Long storeId) {
		SseEmitter emitter = emitterGenerator.setUpSseEmitter(CHANNEL_NAME, emitterMap, storeId);
		...
		try {
			...
			log.warn("[{}] failure send media position data, id={}, {}", CHANNEL_NAME, storeId, e.getMessage());
		}
		return emitter;
	}

	public void unicast(String eventName, Long storeId, StoreOrderEvent storeOrderEvent) {
		Optional.ofNullable(emitterMap.get(storeId))
			.ifPresentOrElse(
				...
				() -> log.warn("[{}] emitter not found, id={}", CHANNEL_NAME, storeId)
			);
	}
